...
let $\|X\|_\Lop$ denote the operator norm (largest singular value),
...
and let $\|X\|_\Lop = \frac{1}{p} \sum_{i=1}^p \|X_{(i)}\|_\Lop$
denote the average operator norm over all $p$ unfoldings.

For a symmetric tensor $X \in (\Re^d)\tp{p}$, let $\cvec(X) \in
\Re^{D_p}, D_p = \binom{d + p + 1}{p}$ be the collapsed vectorization of distinct

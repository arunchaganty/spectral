...
% http://www.cs.sfu.ca/~mori/research/papers/wang_cvpr09.pdf
%Slav Petrov, Discriminative Log-Linear Grammars with Latent Variables
...
%Our goal is to develop consistent estimators which sidestep
...
%mixture of linear regressions (\sectionref{sec:model}).
...
For
$X \in (\Re^d)\tp{p}$ and $Y \in (\Re^d)\tp{(p+q)}$,% $p, q > 0$,
...
%Similar to how $x^T A$ is
%the projection of a vector $x$ onto the row space of a matrix $A$

...
n_2 &= \Omega \left(\epsilon^{-2}~ \frac{k^2 \pi^2_{\max} \|M_3\|_\op^2}{\sigma_k(M_2)^{5} }\frac{ L^{6} S^{6} R^{12}}{\sigmamin(\Sigma_1)^2} \log(1/\delta) \right).
\end{align*}
If the regularization strengths $\lambda_n^{(p)}$ are
set to 

...
moments} $\Pr(x_i^a \mid h_i)$ and $\Pr(x_i^b \mid h_i)$ for each $i$ (\sectionref{directed}).
...
which can be globally optimized using EM (\sectionref{piecewise}).
...
in the undirected case, we need to solve another convex optimization problem (\sectionref{undirected}).
...
%In \sectionref{directed}, we describe our algorithm in more detail and
%show that our algorithm yields consistent parameter estimates
%under non-degenerate rank assumptions.
%describe the

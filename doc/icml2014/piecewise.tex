...
\end{align}
The final form is an expectation over the log of a linear function of
$\mH_\sC$, which is concave in $\mH_\sC$.  
...
Abusing notation slightly, we can think of $M_\sV$ as just a flat multinomial over $d^m$ outcomes
...
$\htz \in \Re^{k-1}$ and $\htm \in \Re^{d-1}$:
\begin{align*}
  \hat\mu &= 
    \begin{bmatrix}
      \htm \\
...
Next, let us compare the two estimators. We use relative efficiency
as a quantitative measure: $e^\mom = \frac{1}{k-1} \Tr(\Sigmaml
...
\begin{lemma}[Relative efficiency with uniform observed moments]
  ...
  > \frac{d+1}{2}$, when $\|\ones_U\|_2 = \frac{d-1}{2}$ the efficiency
  attains its minimum, 
  \begin{align*}
    e^\mom 
...
observed moments are distributed uniformly. The expression describes two
phenomenon: firstly, for a given $k$, the pseudoinverse estimator is
more efficient for small $d$, and secondly, for a given $d$, the

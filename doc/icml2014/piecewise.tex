...
accurate estimates even in general non-asymptotic regimes (see \figureref{cl-hmm}).
...
%  }
  \caption{
    ...
    \todo{change Composite-Likelihood to 'Composite likelihood' in figure}
  Parameter estimation error versus noise in moments $\Pr(x_1,x_2,x_3)$ when recovering parameters for a hidden
  Markov Model with $k=2$ states and $d=5$ observations. Results are averaged over 400 trials.}
    \label{fig:cl-hmm}
\end{figure}

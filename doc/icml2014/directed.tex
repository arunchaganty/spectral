...
If every clique has exclusive views, then we say that $\sG$ does as well.
\end{property}
% PL: to save space
...
\begin{property}[Uniformly bottlenecked]
  ...
  A graphical model $\sG$ is \emph{uniformly bottlenecked} if
  every hidden variable in $\sG$ is a bottleneck.
\end{property}
...
The method also extends directly to case in which the observed variables
  are real valued, while the hidden variables remain discrete. 
In this scenario, the tensor factorization method of
  \citet{anandkumar13tensor} recovers the expected conditional means,
  $\mu_{vi} \eqdef \E[x_v | h_i]$ for an observed variable $x_v$ and
  hidden variable $h_i$.
...
From \citet{anandkumar13tensor} given \assumptionref{full-rank}, we
...
In general, $\sigma_{\min}(\mOpp{v}{i})$ can depend exponentially on the

...
To apply $\TensorFactorize$, we need that each $\mOpp{v}{i}$ has full
  column rank and that each $\pi^{(i)} \eqdef \Pr(h_i) \succ 0$.
...
\begin{align*}
  \|\mOpphat{1}{1} - \mOpp{1}{1}\|_F 
    ...
    &
      ...
      {\sqrt{n}(\sigma_{k}(\mOpp{1}{1}) \sigma_{k}(\mOpp{1}{2}))^{5/2}} \right) \epsilon.
\end{align*}
...
In general, $\sigma_{k}(\mOpp{v}{i})$ can depend exponentially on the

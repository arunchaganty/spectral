...
%      Suppose $$x \not\perp h_1 \wedge h_1 \not\perp h_2 \implies
%      x \not\perp h_2,$$ then $$x \not\perp h_1 \given h \wedge h_1 \not\perp h_2
%      \given h \implies x \not\perp h_2 \given h$$.
  \end{enumerate}
\end{assumption}

%TODO: Describe with example the cliques ruled out.
...
%Let $\sG' = \{ h_{i_1}, \dots, h_{i_m} \} \subseteq \sG$ be
  ...
  assumptions (\assumptionref{ci}) and is uniformly bottlenecked, then

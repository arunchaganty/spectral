...
Let $\Pa(a) \subset X \union H$ be the parents of a variable $a$ in a directed
  model, and 
let $\sN(a) \subset X \union H$ be the neighbors of $a$ in an
  undirected model.
...
We use the notation $[\cdot]$ to indicate indexing; for example $M[i]$
  ...
  of $M$.
For a tensor $T \in \Re^{d \times \ldots \times d}$ and a vector $\bi
  = (i_1, \cdots, i_\ell)$, define the projection $T[\bi] = T[i_1 \ldots
  i_\ell]$.
Similarly, for a set $\sC$ of size $m$ and vector $\bx \in \Re^d$, we
  will use the notation $\bx_\sC$ to get a vector in $\Re^m$ which selects
  the appropriate elements of $\bx$.
...
For an $\ell$-th order tensor $T \in \Re^{d \times \ldots \times
  d}$ and vectors $\bx_1, \cdots, \bx_\ell \in \Re^{d}$, define 
  the application:
\begin{align*}
  T(\bx_1, \cdots, \bx_\ell) 
      %&= \sum_{i_1 = 1}^{d} \cdots \sum_{i_\ell = 1}^{d} 
&= \sum_{\bi = \ones}^{\vec d}
            T[i_1, i_2, \ldots, i_\ell] \bx_1[i_1] \cdots \bx_\ell[i_\ell].
\end{align*}
...
\end{align*}
a tensor in $\Re^{k \times \ldots \times k}$.

...
\begin{frame}
  ...
  \splitcolumn{%
    \begin{itemize}
      \item<1-> Each edge has a parameter.
...
\begin{frame}
  ...
  \begin{itemize}
    \item<3-> {\bf Theorem:} A bottlenecked clique has exclusive views.
  \end{itemize}
  }{%
  \begin{canvas}
    \point{mark}{(4cm,0)};
...
\begin{frame}
  \frametitle{Pseudoinverse Estimator}
  \splitcolumn{%
  \begin{enumerate}
    \item<+-> Estimate conditional moments $\Pr(x|h)$ using bottleneck
      algorithm.
    \item<+-> Solve linear equation between $\Pr(h_1,\cdots,h_m)$ and $P(x_1,
      \cdots, x_m)$.
      \begin{align*}
        Z &= M(\mOppi{1}{1}, \cdots, \mOppi{m}{m}).
      \end{align*}
...
\begin{frame}
  ...
  \splitcolumn{%
    \begin{itemize}
      \item<1-> The MLE is most statistically
        efficient, but hard to compute.
      \item<2-> Given the observed marginals, $-\log \Pr(x)$ is convex in $\theta$.
      \item<3-> A local method like EM is guaranteed to converge to the global optimum.
    \end{itemize}
  }{%
    \begin{canvas}
      \point{stuff}{(2cm,2.5cm)};
      \drawbridge<1->{(stuff)};
      \node<2->[scale=0.8] at ($(start-bridge) - (-1,2.5cm)$) {\obj{
      \begin{align*}
        \log \Pr(\bx) &= \log \sum_{h_1,h_2} \underbrace{\Pr(\bx_1 | h_1) \Pr(\bx_2 | h_2)}_{\text{known}} \mathmb{\Pr(h_1, h_2)}
      \end{align*}
      }};
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Can be recovered by normalization:
      \begin{align*}
        \Pr(h_2 \given h_1) &= \frac{\Pr(h_1, h_2)}{\sum_{h_1} \Pr(h_1, h_2)}.
        \end{align*}
  \end{itemize}
  }{%
    \begin{canvas}
  ...
  \splitcolumn{%
    \begin{itemize}
        \item<1-> Estimating marginals $\mu_\sC$ is independent of treewidth, but 
          computing the normalization constant is:
          \begin{align*}
            A(\theta) &\eqdef \log \sum_{\bx, \bh} \exp\left(\theta^\top \phi(\bx, \bh) \right).
          \end{align*}
        \item<2-> We can use pseudolikelihood to consistently estimate
...
\begin{frame}
  ...
  \splitcolumn{%
    ...
    \item<2-> Efficiently learns models with low-degree but {\bf high-treewidth}.

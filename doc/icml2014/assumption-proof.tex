...
\end{theorem}
\begin{proof}
  The first statement is a result of from Theorem 5.1 of
  \citet{anandkumar13tensor}. The second statement follows directly from
  \lemmaref{mopp-singular-values}.
\end{proof}

\theoremref{sample-complexity} says that the number of samples required
  to estimate the view $\mOpp{v}{i}$ grows exponentially in the {\em
  distance} between $x_v$ and $h_i$. 
Intuitively, this is because the influence from $h_i$ to $x_v$ gets
  diluted for every latent variable it must factor through.

...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \IF{$\sG$ is directed} \STATE Normalize each $\Pr(a,\Pa(a))$. \ENDIF
    \IF{$\sG$ is undirected with low treewidth}
    ...
    \ENDIF
    \IF{$\sG$ is undirected with high treewidth}
      \STATE Compute expected features $\{\mu_S\}_{S \in \sB}$ (\equationref{logLinearFeatures}).
...
A key observation is that for each $B \in \sB(a)$, each $\{a\} \cup B$ is bidependent:

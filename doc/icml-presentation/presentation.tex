...
\begin{frame}
  ...
  \uncover<3->{\cite{tomioka2010estimation}} 
    ...
    \begin{align*}
      \robustaltm<-3>{\hat M_2}{\mathmb{\hat M_3}} &= \arg\min_{M} \sum_{(x,y)\in\mathcal{D}} \left(y^2 - 
      \robustaltm<-3>{\innerp{M}{x\tp{2}}}{\mathmb{\innerp{M}{x\tp{3}}}}
      - \textrm{bias}\right)^2 
      \uncover<3->{+
...
\begin{frame}
  ...
  \begin{canvas}
    \input{spectral-experts.tikz}
    \node[style=txt] at (reg-label) {\robustalt<1>{\color{blue}\bf low-rank}{low-rank} regression};

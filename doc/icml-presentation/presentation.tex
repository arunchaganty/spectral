...
\begin{frame}
  ...
  \begin{tikzpicture}
    % x, y
    \llhood{0}{0};
    \node[scale=0.7] at (mle) {x};
    \node<2-> at ($(mle) + (0.6cm,0)$) {$\mathmb{\theta^*_{\textrm{MLE}}}$};
    \node[scale=0.7] at (em1) {x};
    \node<3-> at ($(em1) + (0.5cm,0)$) {$\mathmr{\hat\theta_{\textrm{EM}}}$};
    \node[scale=0.7] at (em2) {x};
    \node<3-> at ($(em2) + (0.5cm,0)$) {$\mathmr{\hat\theta_{\textrm{EM}}}$};
...
\begin{frame}
  ...
  \begin{align*}
    ...
    \action<5->{%
    y^3 \tikzmark{regC}
    &= \color{blue} \ub{\innerp{\E[\beta_h\tp{3}]}{x\tp{3}}}_{M_3} &&+ \color{DarkGreen} \textrm{bias} + \color{red} \textrm{noise} 
    }
  \end{align*}
  }{%
  }
...
\begin{frame}
  ...
  \uncover<3->{\cite{tomioka2010estimation}} 
    ...
    \begin{align*}
      \robustaltm<-2>{\hat M_2}{\mathmb{\hat M_3}} &= \arg\min_{M} \sum_{(x,y)\in\mathcal{D}} \left(y^2 - 
      \robustaltm<-2>{\innerp{M}{x\tp{2}}}{\mathmb{\innerp{M}{x\tp{3}}}}
      - \textrm{bias}\right)^2 
      \uncover<2->{+
...
\begin{frame}
  ...
  \begin{canvas}
    ...
    \includegraphics<1>[width=\textwidth,height=5cm,keepaspectratio]{figures/1833.pdf}
      ...
      \includegraphics<4-5>[width=\textwidth,height=5cm,keepaspectratio]{figures/Spectral-1833.pdf}
      \includegraphics<6->[width=\textwidth,height=5cm,keepaspectratio]{figures/Spectral-Spectral+EM-1833.pdf}
      };

    \node<1-2>[anchor=west, right=1 cm of em.east] {%
        $y = \beta^T 
            \begin{bmatrix} 
              1 \\
              t \\
    ...
    \node[anchor=west, right=6.2cm of em] at (em.west) {%
      \includegraphics<3-4>[width=\textwidth,height=5cm,keepaspectratio]{figures/EM-hist.pdf}
      \includegraphics<5>[width=\textwidth,height=5cm,keepaspectratio]{figures/EM-Spectral-hist.pdf}
      \includegraphics<6>[width=\textwidth,height=5cm,keepaspectratio]{figures/EM-Spectral-Spectral+EM-hist.pdf}
    };
  \end{canvas}
\end{frame}
...
\begin{frame}
  ...
  \begin{tikzpicture}
    ...
    \node<4->[scale=0.7] at (spec) {x};
    \node<4-> at ($(spec) + (0.5cm,0.3cm)$) {$\mathmg{\hat\theta_{\textrm{spec}}}$};
    \draw<4->[dashed,DarkGreen,line width=0.7pt] ($(spec)-(0.5cm,0)$) -- ($(spec)+(3.5cm,0)$);

    \draw<5->[-latex,smooth,line width=1pt,DarkGreen] ($(spec) + (-0.1cm,+0.05cm)$) -- ($(mle) + (-0.30cm,+0.10cm)$);
    \node<5->[scale=0.7] at (mle) {x};
    \node<5->[anchor=west] at ($(mle) + (0.3cm,0)$) {$\mathmb{\theta^*}_{\textrm{spec + EM}}$};
    \draw<5->[dashed,blue,line width=0.7pt] ($(mle)-(0.4cm,0)$) -- ($(mle)+(0.4cm,0)$);
  \end{tikzpicture}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> Consistent estimator for the mixture of linear regressions with polynomial sample and computational complexity.

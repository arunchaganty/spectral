...
\usepackage{listings}
\usepackage[style=verbose,autocite=footnote,maxnames=10,babel=hyphen,hyperref=true,abbreviate=false,mcite]{biblatex}
...
\begin{frame}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \point{a}{(gen -| mark)};
    \point{b}{(disc -| mark)};
    \node<1->[anchor=north east] at (a) {\includegraphics[width=\textwidth,height=2cm,keepaspectratio]{figures/gen.png}};
    \node<3->[anchor=north east] at (b) {\includegraphics[width=\textwidth,height=2cm,keepaspectratio]{figures/disc.png}};
  \end{tikzpicture}
  }
...
\begin{frame}
  ...
  \twocolumn{%
    \begin{align*}
        \action<1->{%
        \tikzmark{tensoring}x\tp{3}_{ijk} &= x_i x_j x_k \comment{tensoring}\\
        }
        \action<2->{%
        \tikzmark{innerp}\innerp{A}{B} &= \sum_{ijk} A_{ijk} B_{ijk} \comment{inner product}\\
        }
    \end{align*}
  }{%
  \begin{tikzpicture}[remember picture, overlay]
    \draw[use as bounding box] (current page.north west) rectangle (current page.south east);
    \point{mark}{(4cm,0)};
    \point{tensoring}{(pic cs:tensoring)};
    \point{innerp}{(pic cs:innerp)};
    \point{a}{(tensoring -| mark)};
    \point{b}{(innerp -| mark)};
      \tikzcube{black,fill=white}{($(a) + (0,0,0)$)}{1}{1}{1};
      \tikzcube{black,fill=white}{($(b) + (0,0,0)$)}{1}{1}{1};
    \end{tikzpicture}
  }
\end{frame}
...
\begin{frame}
  ...
  \twocolumn{%
    \begin{itemize}
      \item If $\beta_h$ are orthogonal;
      \begin{align*}
        \E[x\tp{3}] &= \sum_{h=1}^k \pi_h \beta_h\tp{3}. 
      \end{align*}
      \item<2-> $\beta_h$ are eigenvectors \footcite{AnandkumarGeHsu2012}!
        \begin{align*}
